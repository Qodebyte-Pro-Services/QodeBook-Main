...
import { getExpenseAnalytics } from "@/api/controllers/get/handler";
import { useEffect, useMemo } from "react";
import { useDonutChartLabels } from "@/store/state/lib/chart-state-manager";

const ExpenseCategoryDonutChart = ({businessId}: {businessId: number}) => {

    const { setDonutChartLabels } = useDonutChartLabels();

    const {data: expenseAnalytics, isSuccess: expenseAnalyticSuccess, isError: expenseAnalyticError} = useQuery({
        queryKey: ["get-expense-analytics", businessId],
        queryFn: () => getExpenseAnalytics({businessId}),
    ...
    const expenseCategories = useMemo(() => {
        if (expenseAnalyticSuccess && !expenseAnalyticError) {
            return expenseAnalytics?.topExpenseCategories;
        }
        return [];
    }, [expenseAnalytics, expenseAnalyticSuccess, expenseAnalyticError]) as Array<{category: string; total_expense: string}>;
    ...
    const chart_data_logic = useMemo(() => {
        ...
        if (expenseCategories?.length) {
            return expenseCategories?.map((item, idx) => ({
                ...
                value: parseFloat(item?.total_expense) || 0,
                fill: `var(${colors[idx]})` || ""
            }));
        };
        return [
            ...
            <PieChart>
                <ChartTooltip
                    ...
                    content={({ active, payload, label }) => {
                        ...
                        const percentage = ((data.value / expenseCategories?.reduce((p, v) => p + +(v?.total_expense), 0)) * 100).toFixed(1);
