...
import { useBudgetHandler } from "@/hooks/useControllers";
...
const CreateBudgetForm = ({ handleFormClose, businessId }: { handleFormClose: () => void; businessId: number; }) => {
  const { hiddenScrollbar } = useCustomStyles();

  const queryClient = useQueryClient();
  const createBudgetMutation = useBudgetHandler();
  ...
  const business_details = useMemo<BusinessReponseLogic[]>(() => {
  ...
  const handleSubmit = async () => {
    ...
    const payload: BudgetPayloadLogic = { ...budget };
    try {
          ...
          <div className="max-h-[65vh] bg-gradient-to-b from-white to-gray-50 px-4.5 md:px-6 pt-5 h-fit overflow-y-auto overflow-x-hidden" style={{ ...hiddenScrollbar }}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div className="flex flex-col gap-y-0.5">
                <div className="text-[12px] md:text-[13px] font-[600]">Business <span className="text-red-500">*</span></div>
                {businessLoading && (
                  <Select>
                    <SelectTrigger className="w-full py-2.5 px-2 border-gray-300 focus:ring-2 focus:ring-template-primary/60">
                      <SelectValue placeholder="Business Loading...." />
                    </SelectTrigger>
                  </Select>
                )}
                {!businessLoading && (
                  <Select value={`${budget.business_id}`} onValueChange={(val) => handleSelectNumber("business_id", val)}>
                    <SelectTrigger className={`w-full py-2.5 px-2 border ${errors.business_id ? "border-red-400" : "border-gray-300"} focus:ring-2 focus:ring-template-primary/60`}>
                    ...
                    <SelectContent>
                      ...
                      {business_details?.map((val, idx) => (
                        <SelectItem key={`budget-business-list-${idx}`} value={`${val?.id}`}>
                          {val?.business_name?.replace(/\b\w/g, (char) => char.toUpperCase())}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                )}
                {errors.business_id ? <span className="text-[11px] text-red-500 mt-1">{errors.business_id}</span> : null}
              </div>
