...
import { getStaffByBusinessId, userBusinessesHandler } from "@/api/controllers/get/handler";
import { useMemo, useState } from "react";
import { useExpenseCategoryHandler } from "@/hooks/useControllers";
...
const CreateExpenseCategory = ({handleFormClose, businessId}: {handleFormClose: () => void; businessId?: number;}) => {
    ...
    const [expenseCategoryError, setExpenseCategoryError] = useState<ExpenseCategoryErrorLogic>({
        business_id: "",
        description: "",
        name: ""
    });
    ...
    const createHandler = useExpenseCategoryHandler();

    const queryClient = useQueryClient();

    const {data: businesses, isLoading: businessLoading, isSuccess: businessSuccess, isError: businessError} = useQuery({
        queryKey: ["businesses"],
    ...
    const business_details = useMemo<Array<BusinessReponseLogic>>(() => {
        if (!businessError && businessSuccess) {
            return businesses?.businesses || [];
        }
        return [];
    }, [businesses, businessSuccess, businessError]);
    ...
    const handleSelect = (name: "business_id", value: string) => {
        setExpenseCategory(prev => {
            return {
                ...prev,
                [name] : +value
        ...
        setExpenseCategoryError(prev => ({
            ...prev,
            [name as keyof ExpenseCategoryErrorLogic]: ""
        }));
    }
    ...
    const validateForm = () => {
        ...
        if (Object.entries(newErrors)?.filter(([_, value]) => Boolean(value)).length) {
            setExpenseCategoryError(prev => ({
                ...prev,
                ...newErrors 
            }));
    ...
    const handleSubmit = async () => {
        ...
        try {
            await createHandler.mutateAsync(payload, {
                onSuccess(data) {
                    toast.success(data?.message || "Expense Category Created Successfully");
                    queryClient.invalidateQueries({
                        queryKey: ["expense-category", payload?.business_id],
                        refetchType: "active"
                    });
                    ...
                    <div className="py-5 px-4 rounded-tl-xl rounded-tr-xl bg-gradient-to-r from-template-primary via-template-primary/80 to-template-primary/40">
                        <div className="flex items-center gap-x-2">
                            ...
                            <div className="space-y-1">
                                <div className="text-base font-[550] text-white">Add Expense Category</div>
                                <div className="text-xs font-[450] text-template-whitesmoke/70 text-shadow-2xs">Use this feature to record a new expense. Fill in the details such as; Category, description, date and amount</div>
                            </div>
                        </div>
                    ...
                    <div className="max-h-[60vh] bg-white px-4.5 pt-5 h-fit overflow-y-auto overflow-x-hidden" style={{...hiddenScrollbar}}>
                        <div className="grid grid-cols-1 gap-3">
                            <div className="flex flex-col gap-y-0.5">
                                <div className="text-[13px] font-[500]">Business</div>
                                {businessLoading && (
                                    <Select>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Available Business's Loading...." />
                                        </SelectTrigger>
                                    </Select>
                                )}
                                {!businessLoading && (
                                    <Select value={`${expenseCategory.business_id}`} onValueChange={val => handleSelect('business_id', val)}>
                                        <SelectTrigger className="w-full py-2.5 px-2">
                                            <SelectValue placeholder="Select A Business" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {business_details?.length <= 0 ? (
                                                <SelectItem value="0">No Available Business&apos;s</SelectItem>
                                            ) : (
                                                <SelectItem value="0">Select A Business</SelectItem>
                                            )}
                                            {business_details?.map((val, idx) => (
                                                <SelectItem key={`expense-category-business-list-${idx}`} value={`${val?.id}`}>{val?.business_name?.replace(/\b\w/g, char => char.toUpperCase())}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                )}
