...
import { StaffRoleTypes } from "@/models/types/shared/handlers-type";
import { useCreateStaffRoleHandler } from "@/hooks/useControllers";
import { toast } from "sonner";
import { useQueryClient } from "@tanstack/react-query";

interface CreateStaffRoleFormProps {
    businessId: string;
    created_by: string;
    handleFormClose: () => void;
}

const CreateStaffRoleForm = ({ businessId, created_by, handleFormClose }: CreateStaffRoleFormProps) => {
    const { hiddenScrollbar } = useCustomStyles();
    const createStaffRoleMutation = useCreateStaffRoleHandler();

    const [formData, setFormData] = useState<Omit<StaffRoleTypes, 'business_id'>>({
        role_name: "",
        permissions: [],
        description: "",
        created_by: ""
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload: StaffRoleTypes = {
                ...formData,
                created_by,
                business_id: parseInt(businessId)
            };

            await createStaffRoleMutation.mutateAsync(payload, {
                onSuccess: async (data) => {
                    toast.success(data?.message ?? "Staff Role Created Successfully");
                    await queryClient.invalidateQueries({ queryKey: ['get-staff-roles', Number(businessId)], refetchType: "active" });
                    handleFormClose();
                },
                onError: (err) => {
                    console.error('Error creating staff role:', err);
                    toast.error((err instanceof Error && (err?.message?.includes("permission") ? "You're not authorized for this action" : err?.message)) ?? "An unexpected error occurred while creating staff role");
                }
            });
        } catch (error) {
                ...
                <div className="bg-gradient-to-r from-template-primary to-template-primary/80 p-6 rounded-t-lg">
                    <div className="flex items-center justify-between text-white">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-[600]">Create Staff Role</h2>
                                <p className="text-sm opacity-90">Define role permissions and access levels</p>
                            </div>
                        </div>
                        <button
                    ...
                    <div className="max-h-[60vh] overflow-auto" style={hiddenScrollbar}>
                        <div className="space-y-6">
                            <div className="flex flex-col gap-y-2">
                                ...
                                <input
                                    type="text"
                                    className={`py-3 px-4 text-sm w-full border rounded-lg focus:outline-none focus:ring-2 focus:ring-template-primary/20 focus:border-template-primary transition-colors ${errors.role_name ? 'border-red-500' : 'border-gray-300'
                                        }`}
                            ...
                            <div className="flex flex-col gap-y-2">
                                ...
                                <textarea
                                    className={`py-3 px-4 text-sm w-full border rounded-lg focus:outline-none focus:ring-2 focus:ring-template-primary/20 focus:border-template-primary transition-colors resize-none ${errors.description ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                    rows={3}
                            ...
                            <div className="flex flex-col gap-y-2">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className={`w-full py-3 px-4 text-left border rounded-lg focus:outline-none focus:ring-2 focus:ring-template-primary/20 focus:border-template-primary transition-colors flex items-center justify-between ${errors.permissions ? 'border-red-500' : 'border-gray-300'
                                            }`}
                                    >
...
export default CreateStaffRoleForm;
