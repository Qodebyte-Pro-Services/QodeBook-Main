"use client";
import { useSuppliersHandler } from "@/hooks/useHandlers";
...
const SuppliersForm = ({handleFormClose, business_id}: {handleFormClose: () => void, business_id: number}) => {
    ...
    const supplierHandler = useSuppliersHandler();

    const validateInputs = (name: string, value: string): string => {
        switch(name) {
            case "name":
                return !value.trim() ? "Supplier's name is required" : "";
            case "contact":
    ...
    const handleSupplier = async () => {
        ...
        try {
            await supplierHandler.mutateAsync(formdata, {
                onSuccess: (data) => {
                ...
                onError: (err) => {
                    console.log(err);
                    toast.error(err?.message ?? "An unexpected error occurred while creating Supplier");
                }
            });
        }catch(err) {
            console.log(err);
                    ...
                    <div className="bg-gradient-to-r from-template-chart-store to-template-primary p-6 text-white">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-xl font-bold">Create New Supplier</h2>
                            </div>
                            <button 
                        ...
                        <p className="text-white/80 text-sm">
                            Add a new Supplier to organize your products effectively
                        </p>
                    </div>
