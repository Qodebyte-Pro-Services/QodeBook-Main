...
const InventoryTable = ({data, isLoading, businessId, refetchProduct}: {data: Array<StoreProductTask>; isLoading: boolean; businessId: number; refetchProduct: () => void}) => {
    ...
    }, [categories]); //! Shaa, incase the categories is then passed as a string, i can then switch to category as well
    ...
    <Card className="dark:bg-black">
        <div className="flex flex-col gap-y-3">
            ...
            <div className="px-4 py-5 bg-white dark:bg-black rounded-sm">
                <DataTableWithNumberPagination columns={columns} data={productsData} filterId="name" placeholderText="Search by inventory name/ID..." isShowCost={false} isShowStock={true} displayedText="Products" isLoading={isLoading} />
            </div>
        </div>
    </Card>
  );
