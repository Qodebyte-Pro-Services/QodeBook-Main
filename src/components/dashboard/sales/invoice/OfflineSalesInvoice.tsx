...
const OfflineSalesInvoice = ({ sale, onClose, onPrint }: SalesInvoiceProps) => {
    ...
    const handleAction = useCallback(
        async (action: "print" | "pdf" | "image" | "share") => {
    ...
    const taxTotal = sale?.taxes || 0;
    const discountTotal = sale?.discount || 0;
    const total =
        sale?.payments
            ?.map((val) => val?.amount)
            ?.reduce((prev, val) => (prev += val), 0) || 0;
    ...
    return (
        <div
            className="fixed inset-0 min-[520px]:top-0 min-[520px]:left-1/2 min-[520px]:-translate-x-1/2 print:left-0 print:-translate-x-0 z-50 w-full min-[500px]:w-[450px] py-3 px-6 bg-white print:p-0 print:w-auto print:h-auto print:relative print:inset-auto"
            id="receipt-print">
            <div
                ref={receiptRef}
                className="flex flex-col gap-y-2 bg-white p-4 print:p-2">
                {/* Header */}
                <div className="text-center">
                    <div className="text-base font-bold">RECEIPT</div>
                    <div onClick={onClose} className="text-xs text-gray-500">
                        #
                        {new Date(sale?.createdAt)?.getTime()?.toString(16) +
                            "-" +
                            sale?.id}
                    </div>
                </div>
                ...
                <div
                    ...
                    {/* Order Info */}
                    <div className="mb-2">
                        <div className="text-xs font-semibold text-gray-700 border-b border-gray-200 pb-1 mb-1">
                            ORDER DETAILS
                        </div>
                        <div className="grid grid-cols-2 gap-y-1 text-xs">
                            ...
                            <div className="text-right capitalize">
                                <span className="bg-amber-200 text-amber-700 px-3 py-1 rounded-full">{sale?.status?.slice(0, sale?.status?.indexOf('_')) || "Completed"}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-2">
                        ...
                        {sale?.items?.length ? (
                            sale?.items?.map((item, index) => (
                                <div
                                    key={`offline-sales-item-${index}`}
                                    className="text-[11px] py-1 border-b border-gray-100 flex">
                    ...
                    <div className="text-[10px] text-gray-500">
                      {item.attributes.map(attr => `${attr.name}: ${attr.value}`).join(', ')}
                                    ...
                                    <div className="w-[15%] text-center self-center">
                                        {item.quantity}
                                    </div>
                            ...
                            <div className="text-xs font-[500]">
                                No Invoice Items Record Detected
                            </div>
                        )}
                    </div>
                    ...
                    <div className="mb-2">
                        ...
                        <div className="text-xs space-y-1">
                            ...
                            {taxTotal > 0 && (
                                <div className="flex justify-between">
                                    <span>
                                        Tax {((sale?.taxes * 100) / (sale?.payments?.length > 1 ? sale?.payments?.map(item => item?.amount)?.reduce((prev, itm) => prev += itm, 0) : sale?.payments?.[0]?.amount))?.toFixed(2)}%:                                        
                                    </span>
                                    <span>
                                        {new Intl.NumberFormat("en-NG", {
                                            style: "currency",
                    ...
                    <div className="flex flex-col gap-3 pt-4 print:hidden">
                        ...
                        <Button
                            onClick={onClose}
                            variant="outline"
                            className="h-11 text-sm font-medium mt-1"
                            disabled={isProcessing}>
                            Close
                        </Button>
                    </div>
