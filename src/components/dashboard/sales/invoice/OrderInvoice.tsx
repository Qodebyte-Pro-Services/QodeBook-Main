...
const OrderInvoice = ({ orderData, onClose, onPrint, onDownload }: OrderInvoiceProps) => {
...
const handleDownload = useCallback(
  async (type: "pdf" | "image" = "pdf") => {
    ...
    try {
      ...
      if (type === "image") {
        ...
        link.download = `receipt-${orderData.sale.id}-${Date.now()}.png`;
        link.href = dataUrl;
        link.click();
        ...
        // Load the generated image into PDF
        ...
        pdf.save(`receipt-${orderData.sale.id}-${Date.now()}.pdf`);
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: `Receipt #${orderData.sale.id}`,
          text: `Receipt for order #${orderData.sale.id}`,
        ...
        return await handleDownload('image');
      }
    } catch (error) {
  ...
  }, [handleDownload, orderData.sale.id]);
  ...
  }, [isProcessing, handlePrint, handleDownload, handleShare, resetActiveAction]);
  ...
  // Calculate totals
  const subtotal = parseFloat(orderData.sale.subtotal);
  const taxTotal = parseFloat(orderData.sale.tax_total);
  const discountTotal = parseFloat(orderData.sale.discount_total);
  const total = parseFloat(orderData.sale.total_amount);
  const totalPaid = orderData.payments.reduce((sum, payment) => sum + parseFloat(payment.amount), 0);
    ...
    >
      <div ref={receiptRef} className="flex flex-col gap-y-2 bg-white dark:bg-black p-4 print:p-2">
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">#{orderData.sale.id}</div>
        </div>
        
        {/* Logo and Barcode */}
        <div className="border py-1 px-4 flex flex-col gap-y-0.5 items-center my-2" style={{ borderColor: "#40922c" }}>
          <Image 
            width={150} 
            height={150} 
        ...
        <div className="max-h-[60vh] overflow-y-auto py-2" style={{ scrollbarWidth: "none" }}>
          {/* Order Info */}
          <div className="mb-2">
            <div className="text-xs font-semibold text-gray-700 border-b border-gray-200 pb-1 mb-1">ORDER DETAILS</div>
            <div className="grid grid-cols-2 gap-y-1 text-xs">
              <div className="text-gray-500">Date:</div>
              <div className="text-right">{formatDate(orderData.sale.created_at)}</div>
              <div className="text-gray-500">Order Type:</div>
              <div className="text-right capitalize">{orderData.sale.order_type || 'Retail'}</div>
              <div className="text-gray-500">Status:</div>
              <div className="text-right capitalize">{orderData.sale.status || 'Completed'}</div>
            </div>
          </div>
          
          {/* Customer Info */}
          {orderData.sale.customer_name && (
            <div className="mb-2">
              <div className="text-xs font-semibold text-gray-700 border-b border-gray-200 pb-1 mb-1">CUSTOMER</div>
              <div className="grid grid-cols-2 gap-y-1 text-xs">
                <div className="text-gray-500">Name:</div>
                <div className="text-right">{orderData.sale.customer_name}</div>
                {orderData.sale.customer_phone && (
                  <>
                    <div className="text-gray-500">Phone:</div>
                    <div className="text-right">{orderData.sale.customer_phone}</div>
                  </>
                )}
                {orderData.sale.customer_email && (
                  <>
                    <div className="text-gray-500">Email:</div>
                    <div className="text-right truncate">{orderData.sale.customer_email}</div>
                  </>
                )}
              </div>
            </div>
          ...
          {/* Items Table */}
          <div className="mb-2">
            <div className="text-xs font-semibold text-gray-700 border-b border-gray-200 pb-1 mb-1">ITEMS</div>
            <div className="text-[11px] border-b border-gray-200 pb-1 mb-1 font-semibold flex">
            ...
            {orderData.items.map((item, index) => (
              <div key={`${item.id}-${index}`} className="text-[11px] py-1 border-b border-gray-100 flex">
          ...
          {/* Payment Summary */}
          <div className="mb-2">
            <div className="text-xs font-semibold text-gray-700 border-b border-gray-200 pb-1 mb-1">PAYMENT SUMMARY</div>
            <div className="text-xs space-y-1">
              ...
              {taxTotal > 0 && (
                <div className="flex justify-between">
                  <span>Tax ({orderData.sale.tax_total || 0}%):</span>
                  <span>
                    {new Intl.NumberFormat("en-NG", {
                      style: "currency",
                      currency: "NGN"
              ...
              <div className="border-t border-gray-200 pt-1 mt-1 font-semibold">
                <div className="flex justify-between">
                  <span>Total:</span>
                  <span>
                    {new Intl.NumberFormat("en-NG", {
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      disabled={isProcessing}
                      variant="default"
                      className="h-11 gap-2 text-sm font-medium bg-[#40922c] hover:bg-[#368023]"
                    >
