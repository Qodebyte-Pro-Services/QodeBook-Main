...
import { useStaffSubchargeUpdate } from "@/hooks/useControllers";
import { StaffSubchargeResponse } from "../sections/staff-subcharge";
...
type SubchargeFormLogic = {
    ...
    staff_subcharges: StaffSubchargeResponse & Partial<StaffResponseLogic>;
}

const EditStaffSubcharges = ({handleFormClose, staff_subcharges}: SubchargeFormLogic) => {
    const [subcharges, setSubcharges] = useState<SubchargesPayloadLogic>(() => ({
        business_id: staff_subcharges?.business_id || 0,
        staff_id: staff_subcharges?.staff_id || "",
        sub_charge_amt: +staff_subcharges?.sub_charge_amt || 0,
        reason: staff_subcharges?.reason || ""
    }));
    ...
    const staffSubchargeHandler = useStaffSubchargeUpdate();
    ...
    const handleSubchargeSubmit = async () => {
        ...
        try {
            ...
            await staffSubchargeHandler.mutateAsync(payload, {
                onSuccess(data) {
                    toast.success(data?.message || "Staff Subcharge Updated Successfully");
                    setIsSubmitting(false);
                    queryClient.invalidateQueries({
                        queryKey: ["get-staff-subcharges", payload?.business_id],
                        refetchType: "active"
                    });
                    setTimeout(() => {
                        handleFormClose?.();
                ...
                onError(err) {
                    toast.error(err?.message || "Error Occurred While Trying To Update Staff Subcharges");
                    setIsSubmitting(false);
                    setTimeout(() => {
                        handleFormClose?.();
                    }, 2000);
                    ...
                    <div className="bg-gradient-to-r from-template-chart-store to-template-primary p-6 text-white">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-xl font-bold">Edit Staff Subcharge</h2>
                            </div>
                            <button 
...
export default EditStaffSubcharges;