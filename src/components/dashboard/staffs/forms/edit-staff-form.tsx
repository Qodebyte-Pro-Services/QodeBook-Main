...
import { useStaffCreation, useUpdateStaffListform } from "@/hooks/useHandlers";
import { getStaffRoles, getUserDetails } from "@/api/controllers/get/handler";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { StaffResponseLogic, StaffRoleObj } from "@/models/types/shared/handlers-type";
...
export default function EditStaffForm({ onClose, businessId, branchId, staffList }: CreateStaffFormProps) {
  const [formData, setFormData] = useState<Omit<StaffFormDataLogic, 'staff_id' | 'business_id' | 'branch_id'>>(() => ({ ...staffList }));
  ...
  const staffCreateHandler = useUpdateStaffListform();
  ...
  const handleSelectChange = (name: keyof Pick<StaffFormDataLogic, "assigned_position" | "position_name" | "employment_type" | "staff_status" | "gender" | "payment_status">, value: string) => {
    if (name && value) {
      setFormData(prev => ({
        ...prev,
        [name]: value
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.assign(formData, { position_name: staff_position_name ?? "N/A" });
      ...
      await staffCreateHandler.mutateAsync(server_payload, {
        ...
        onError(err) {
          console.log(err);
          toast.error("Error Occurred while trying to update staff: " + (err instanceof Error ? err?.message : "...."));
        }
      })
    } catch (error) {
          ...
          <div className="bg-gradient-to-r from-green-600 to-green-800 p-6 text-white">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold">Edit Staff Form</h2>
              </div>
              <button
                onClick={onClose}
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={formData?.staff_status} onValueChange={(value) => handleSelectChange('staff_status', value)}>
                        <SelectTrigger className="w-full py-5.5">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        type="text"
                        name="staff_status_change_reason"
                        value={formData.staff_status_change_reason || ''}
                        onChange={handleInputChange}
                        className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition"
                        placeholder="Reason for status change (if any)"
                      />
                    </div>
                  </div>
                </motion.div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={formData?.payment_status} onValueChange={(value) => handleSelectChange('payment_status', value)}>
                        ...
                        <SelectContent className="z-[10000]">
                          ...
                          <SelectItem value="un_paid">Un Paid</SelectItem>
                          <SelectItem value="paid_half">Paid Half</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    {/* <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={new Date(formData?.last_payment_date as string || "")?.toISOString()?.split("T")[0] || ''}
                          onChange={handleInputChange}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition"
                        />
                      </div>
